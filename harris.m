...
% set defaults
param.disp = 1;      % overlay corner points on image
param.N = 2000;       % number of interest points to return
param.subpixel = false;  % refine corner location with subpixel accuracy
param.thresh = 5;    % threshold value for smallest response magnitude
param.hsize = 4;     % size of gaussian smoothing mask
param.sigma = 0.5;   % standard deviation of gaussian mask
param.eig   = false; % use corner response function as originally
                     % proposed by Harris
param.tile = [10 10];  % do not process image regionally
